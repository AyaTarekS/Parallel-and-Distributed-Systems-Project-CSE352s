...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  if (transactionId) {
    try {
      const response = await fetch(`http://localhost:3000/transactions/byId/?transactionId=${transactionId}`);
      const transaction = await response.json();

      const userId = localStorage.getItem('user_id')
      const response2 = await fetch(`http://localhost:3000/buyerAccount/?user_id=${userId}`);
      const buyerInfo = await response2.json();
   
      const response3 = await fetch(`http://localhost:3000/productsDetails/?user_id=${userId}`);
      ...
      const response4 = await fetch(`http://localhost:3000/orderContents/?order_id=${matchedorder.order_id}`);
      ...
      const response5 = await fetch(`http://localhost:3000/paymentInfo/?order_id=${matchedorder.order_id}`);
